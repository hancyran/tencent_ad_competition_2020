...
class Model(BaseModel):
    ...
    def build_graph(self, hparams):
        ...
        # Pool
        first_output_rnn = tf.concat(outputs, axis=2)

        # Second RNN Layer: LSTM
        rnn_cell = rnn.BasicLSTMCell(self.hidden_size * 2)
        rnn_cell=rnn.DropoutWrapper(rnn_cell,output_keep_prob=hparams.dropout_keep_prob)
        # dynamic rnn
        _, final_state_c_h = tf.nn.dynamic_rnn(rnn_cell, first_output_rnn, dtype=tf.float32)
        second_output_rnn = final_state_c_h[1]

        output = tf.layers.dense(second_output_rnn, hparams.hidden_size * 2, activation=tf.nn.tanh)
        ...
        self.output_logits = tf.matmul(output, self.W_projection) + self.b_projection
